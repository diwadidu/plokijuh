import { SetCollection, isSeq, isSet } from 'immutable';
import { flattenFactory, reify }       from './Operations';
import { IS_SET_SYMBOL }               from './peer-ports/constants';

export class Option extends SetCollection {
  constructor(value) {
    const instance = value === null || value === undefined ? new None() : new Some(value);
    return instance;
  }
  ...
  __iterate(fn, reverse) {
    return !Array.isArray(this._val) ? this._val : this._val.map(k => fn(k, k, this), reverse);
  }
}
